...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Bitultra.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Bitultrad.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBitultraamount=<n> " + _("Keep N Bitultra anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitultra is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitultra is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bitultra version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bitultra server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitultra") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bitultra to complete") << "\n";
    ...
    nAnonymizeBitultraAmount = GetArg("-anonymizeBitultraamount", 0);
    if(nAnonymizeBitultraAmount > 999999) nAnonymizeBitultraAmount = 999999;
    if(nAnonymizeBitultraAmount < 2) nAnonymizeBitultraAmount = 2;
    ...
    LogPrintf("Anonymize Bitultra Amount %d\n", nAnonymizeBitultraAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BTU+1000 == (.1BTU+100)*10
       10BTU+10000 == (1BTU+1000)*10
    */
