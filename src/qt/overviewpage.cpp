...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitultraAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitultraAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitultraAmount = strAnonymizeBitultraAmount.remove(strAnonymizeBitultraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitultraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitultraAmount*COIN) nMaxToAnonymize = nAnonymizeBitultraAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitultraAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitultraAmount));
        strAnonymizeBitultraAmount = strAnonymizeBitultraAmount.remove(strAnonymizeBitultraAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitultraAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
