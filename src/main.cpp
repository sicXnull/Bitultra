...
unsigned int nStakeMinAge = 60 * 60; // 8 hours
...
const string strMessageMagic = "Bitultra Signed Message:\n";
...
bool IsMNCollateralValid(int64_t value, int nHeight) {
    ...
    // Using BOOST_FOREACH for concistency with the rest of the code, everything should be using a plain for from c++ 11 or 17
		if( nHeight < TIERED_MASTERNODES_START_BLOCK_1 )
		{
		...
		else if( nHeight < TIERED_MASTERNODES_START_BLOCK_2 )
		{
...
int64_t GetMNCollateral(int nHeight, int tier) {
    if (nHeight < TIERED_MASTERNODES_START_BLOCK) {
		...
		{
			return masternodeTiers1[tier];
		}
		else
		{
			return masternodeTiers2[tier];
		}
    }
}
...
bool IsPOSRewardValid(int64_t value, int64_t nFees) {
    ...
    if (nHeight < 100) {
        ...
        if (nHeight < TIERED_MASTERNODES_START_BLOCK) {
			...
			{
				BOOST_FOREACH(PAIRTYPE(const int, int)& tier, masternodeTierRewards1)
				{
					if (value == (tier.second*COIN + POS_REWARD_TIERED_MN*COIN + nFees))
						return true;
				}
			...
			{
				BOOST_FOREACH(PAIRTYPE(const int, int)& tier, masternodeTierRewards2)
				{
					if (value == (tier.second*COIN + POS_REWARD_TIERED_MN_2*COIN + nFees))
						return true;
				}
			}
            // The case of a wallet staking with no mns up
			if (nHeight < TIERED_MASTERNODES_START_BLOCK_2)
			{
			...
			{
				if (value ==  POS_REWARD_TIERED_MN_2*COIN + nFees) {
                return true;
				}
			}
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 > 100 && pindexBest->nHeight+1 < TIERED_MASTERNODES_START_BLOCK)
    {
        nSubsidy = STATIC_POS_REWARD;
    }    
	return nSubsidy + nFees;
...
bool CBlock::AcceptBlock()
{
    ...
    if (hashBestChain == hash)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodes)
            if (nBestHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 100 : nBlockEstimate))
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION || (nBestHeight >= FORK1_BlOCK && pfrom->nVersion < MIN_PEER_PROTO_VERSION_FORK1))
        {
